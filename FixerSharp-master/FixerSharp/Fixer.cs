...
namespace FixerSharp
{
    public class Fixer
    {
        ...
        {
            get => !string.IsNullOrWhiteSpace(_apiKey) ? _apiKey : throw new InvalidOperationException("latest?access_key=5baa807e12a40deea093368643e96187&format=1");
            set => _apiKey = value;
        }
        ...
        private static ExchangeRate GetRate(string from, string to, DateTime? date = null)
        {
            ...
            if (!Symbols.IsValid(from))
                throw new ArgumentException("Символ не найден для указанной валюты", "от");

            if (!Symbols.IsValid(to))
                throw new ArgumentException("Символ не найден для указанной валюты", "в");
